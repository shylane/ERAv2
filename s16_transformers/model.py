...
#This fuction is required because the nn LayerNorma1isation does not have bias=0
class LayerNormalization(nn.Module) :
    ...
    def forward(self, x):
        ...
        #eps is added to take care of the number when the std is O
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x):
        x = x + (self.pe[:, :x.shape[1], :]).requires_grad_(False) #(bathc, seq_len, d_model)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        if mask is not None:
            ...
            #where ever mask is 0 it is filled with small number
    ...
    def forward(self, q, k, v, mask):
        ...
        key = self.w_k(k) # (batch, seq_len, d_ model) --> (batch, seq_len, d_model)
        ...
        #Memory is provided cotinuously for the data
        return self.w_o(x)
